...
import { Bug, CheckCircle, Clock, TrendingUp } from 'lucide-react';
...
export default async function DashboardPage() {
  ...
  // Fetch recent bugs with project name
const { data: recentBugs, error } = await supabase
  .from('bugs')
  .select(`
    id,
    bug_number,
...
console.log('recentBugs', recentBugs, 'error', error);


  // Hitung bugs by severity
  ...
  // Group by severity
  const severityCounts = severityStats?.reduce((acc: any, bug) => {
...
console.log('recentBugs', recentBugs, 'error', error);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fadeInUp">
          ...
          {/* Bugs by Severity */}
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-indigo-100 p-6 animate-fadeInUp">
          ...
          {/* Recent Bugs */}
          <div className="lg:col-span-2 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-indigo-100 p-6 animate-fadeInUp">
              ...
              {recentBugs && recentBugs.length > 0 ? (
                recentBugs.map((bug, index) => (
                  <Link
                    ...
                    style={{
                      animation: `fadeIn 0.3s ease-in-out ${index * 0.1}s backwards`,
                    }}
                  >
  ...
  {/* Project name */}
  <span className="text-xs text-gray-500 truncate" title={bug.projects?.name || "Unknown Project"}>
    {bug.projects?.name || "Unknown Project"}
  </span>

  {/* Bug code */}
  <span className="text-xs font-mono font-bold text-indigo-600">
    SCB-{bug.projects?.project_number || "01"}-{String(bug.bug_number).padStart(3, "0")}
  </span>
