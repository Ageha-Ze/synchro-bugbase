...
export default function ProjectsPage() {
  ...
  const fetchRecentBugs = async (): Promise<RecentBugs[]> => {
    try {
      ...
      const mapped: RecentBugs[] = data.map((bug) => ({
        ...bug,
        project: projectsData?.find((p: any) => p.id === bug.project_id) ?? null,
      }));
  ...
  const SortIcon = ({ field }: { field: keyof Project }) => {
    if (sortField !== field) return <ChevronDown className="w-4 h-4 text-gray-400 dark:text-gray-500" />;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        >
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl sm:text-4xl font-bold mb-1 text-white">Projects</h1>
              <p className="text-indigo-100/90 text-sm sm:text-base">Manage and track all your projects easily</p>
              <div className="mt-4">
        ...
        {projects.length === 0 ? (
          <div className="bg-white/80 dark:bg-gray-900/70 backdrop-blur-sm rounded-2xl shadow-md border border-gray-200 dark:border-gray-700 p-12 text-center transition-colors">
            ...
            <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-3">No projects yet</h3>
            <p className="text-gray-600 dark:text-gray-300 mb-6">Create your first project to start tracking bugs</p>
            <Button
              onClick={() => setShowModal(true)}
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {sortedProjects.map((p) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-5 text-gray-600 dark:text-gray-300 text-sm line-clamp-2">{p.description || "—"}</td>
