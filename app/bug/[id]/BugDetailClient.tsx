  ...
  useEffect(() => {
    ...
    const fetchBugData = async () => {
      try {
        ...
        const [bugResult, attachmentsResult] = await Promise.all([
    ...
    const channel = (supabase as any)
      ...
      .on?.(
        ...
        async () => {
          console.log("🔄 Comment changed, refetching...");
          await fetchComments();
        }
      )
      .subscribe?.();
...
const handleAddComment = async () => {
  ...
  try {
    ...
    // Gunakan 'author' bukan 'user_id' sesuai schema tabel
    const { data: newCommentData, error: insertError } = await supabase
      .from("comments")
      .insert([{
        bug_id: bug.id,
        content: newComment.trim(),
        author: user.email || user.id, // bisa pakai email atau username
      }])
      .select(`id, bug_id, content, created_at, author`)
      .single();
      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* ...rest of Main content (Details, Attachments, Comments) ... */}
          </div>
